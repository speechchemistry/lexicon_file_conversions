...
# already pinned by its own <name>-table_end-to-end.R snapshot test, now run
...
test_that("lift2csv_end-to-end_table-and-table-dir-together_errors", {
...
test_that("lift2csv_end-to-end_neither-flag_errors", {
  lift_file <- file.path(entry_fixture_dir, "lela-teli-empty-lexicon.lift")
  expect_usage_error(lift_file, "Supply either --table")
})

test_that("lift2csv_end-to-end_unrecognised-table-name_errors_and_lists_valid_names", {
  lift_file <- file.path(entry_fixture_dir, "lela-teli-empty-lexicon.lift")

...
**Status: A1, A2, T1, T2, T3, B1 and B2 done — Phase B complete. Next: Phase C (C1).** Keep this file synced as remaining [decisions below](#decisions-to-settle) land, per [AGENTS.md's Working Style](../AGENTS.md).
...
**Note (added by [`plans/per-table-script-consolidation.md`](per-table-script-consolidation.md)'s R3):** every test file named `<x>_end-to-end.R` in the sections above has since been renamed to `<x>.R`, and every `_snaps/<x>_end-to-end/` directory to `_snaps/<x>/` — the suffix correlated with "owns a snapshot directory" more than with actually being end-to-end (`test-csv2lift_table-discovery.R` never had it despite being fully end-to-end). This paragraph and the rest of Phase T's narrative above describe what those files were called at the time and are left as written, per [AGENTS.md's Markdown Conventions](../AGENTS.md#markdown-conventions); the live filenames used going forward (Phase B onward) are the renamed ones.
...
**B1 · `sense/reversal` → reversal table (`--reversals`). Done.** Sense-parented, positionally keyed: `zhi-two-pronunciations-with-audio-and-ipa.lift` has one sense with **two reversals of the same `@type="en"`**, so no `reversal_<type>` column scheme works. Copy the example table wholesale — it is the same shape one level up (`R/example_table.R`, `R/example_helpers.R`, `R/csv2lift_example.R`, `test-example-table.R`), keying rows on document position with `sense_guid` as the surviving FK. Columns: `sense_guid`, `reversal_type`, `reversal_<lang>`; classifier starts strict (no custom-field fallback). Attach after `attach_senses_to_lift()`.

Note the pre-existing coverage hole this closes: `_snaps/csv2lift/zhi-note-and-phonology-notes.lift` and `zhi-two-pronunciations-with-audio-and-ipa.lift` are approved snapshots holding **zero** `<reversal>` while their sources have 2 and 3 — exactly [the skill's "a new table opens a coverage hole"](../.claude/skills/adding-a-lift-field/SKILL.md#if-it-needs-a-table-of-its-own) case. Both stems need a `reversals.csv` companion in their export directory.

Since [T1](#phase-t--the-table-folder-convention), adding that companion is a **plain file drop** — `test-csv2lift.R` discovers whatever tables an export directory holds, so no test file is edited to bring a new table into an existing stem's round trip. The flip side is that the file's mere *presence* changes an approved snapshot, where previously a test-loop edit made the intent explicit. So this is a moment to read the `.new` diff rather than accept it: the expected change is `<reversal>` elements appearing where the source always had them, and nothing else moving.

Built as designed above, plus two things the plan didn't anticipate. First, real data forced a decision the plan didn't flag: `<reversal>`'s own schema content model (`reversal-content`) has no uniqueness assertion on `(type, lang)` at all — unlike `note-content`/`field-content` — so the two-same-type-reversal case isn't a fixture accident, it's schema-legal by design, confirmed against `resources/lift-0.13.rng`'s historical text. Second, adding `reversals.csv` to `zhi-note-and-phonology-notes/`'s export directory broke an unrelated test's premise: `test-csv2lift_table-discovery.R`'s override test truncated that stem's `senses.csv` on the explicit (now-false) assumption that the directory had "no examples, so truncating carries no risk" — it now has a `reversals.csv` referencing two of those three senses, and the truncated override dropped both. Fixed the truncation to keep whatever sense_guids any discovered CSV in the directory actually depends on, rather than an arbitrary row prefix, so the same class of break can't recur when the next sense-parented table lands. Also updated `test-lift2csv.R`'s hardcoded expected-filename list for `zhi-two-pronunciations-with-audio-and-ipa.lift`, which now legitimately writes a `reversals.csv` under `--table-dir`.

`<reversal>` is emitted unconditionally, even for an all-blank row, matching `<example>`'s behaviour rather than `<pronunciation>`'s — chosen from the table's own positional-identity model (per [SPEC.md's Structural Rules](../SPEC.md#structural-rules-csv2lift-direction)), since no fixture has a blank or interleaved-blank reversal to prove it against either way. Registered in `R/table_registry.R` after `examples`, matching `lift.rng`'s own declared child order for `sense-content` (confirmed by re-reading the historical schema), though no fixture has a sense with both to prove the ordering against real FLEx output. SPEC.md gained its own [Reversal Table](../SPEC.md#reversal-table) section (modelled on [Pronunciation Table](../SPEC.md#pronunciation-table)), the Sense Table's canonical-child-order and attach-order paragraphs were extended, the cross-reference under Column Classification's known-limitation bullet was added (`type` colliding with `reversal_type`, mirroring `example_source`/`translation_type`), and `<reversal>` was removed from Not Yet Specified's sense-level-traits bullet in favour of its own entry for the two things still unbuilt within it — the nested `<main>` reversal-index hierarchy and reversal-level `<grammatical-info>`, neither present in any fixture. The skill's stale "not-yet-built" cross-reference to `sense/reversal` was corrected to point at the new section instead.

**B2 · `entry/etymology` → etymology table (`--etymologies`). Done.** Built as designed above: entry-parented, positionally keyed (no id/guid on `<etymology>`, same situation as `<pronunciation>`), one row per etymology. `R/etymology_table.R`/`R/etymology_helpers.R`/`R/csv2lift_etymology.R` copy the pronunciation/reversal shape — `extract_etymology_multitext()` is a single helper for both `<form>` and `<gloss>` (both carry `lang` on the node the xpath itself returns, unlike the `<form>`-wrapped shapes elsewhere), and `extract_etymology_multitext_with_attribute()` is the position-keyed adaptation of `extract_multitext_with_attribute()` for the inner `<field type>` children. Unlike pronunciation/reversal, the classifier ends in the same last-underscore custom-field fallback as `classify_entry_columns()`/`classify_sense_columns()`, since etymology-level `<field>` (`comment`, `languagenotes`) is read and written.

Two things the plan didn't anticipate. First, `@type`/`@source` are the first attributes in this codebase with no `<optional>` wrapper in their content model (`etymology-content` requires both) — so `attach_etymologies_to_lift()` always emits them, even as `""`, rather than the "omit when blank" rule every other attribute follows; `etymology_source` is `""` on all 105 `sena3.lift` etymologies, so `drop_empty_columns()` (`R/table_csv.R`) removes it from lift2csv's own output entirely, meaning the write side has to guard on **column absence**, not just blankness, exactly like `entry_order`'s existing guard — this was caught by reading the read-direction's own snapshot before wiring up csv2lift, not by a failing test. Second, real data forced the "omit a blank row?" question that reversal's fixtures never raised: 48 of 105 `sena3.lift` etymologies carry no `<form>` at all (the etymon is recorded only in the `comment` custom field), so `<etymology>` is emitted unconditionally, even all-blank, on the same positional-identity reasoning as `<example>`/`<reversal>` — but here it's the ordinary case, not a hypothetical.

Fixture: `sena3-gloss-initial-b.lift` (already in the repo) covers 5 of the 105 etymologies but, per [the skill's "presence is not coverage" warning](../.claude/skills/adding-a-lift-field/SKILL.md#get-a-real-fixture), only exercises `type="borrowed"` and the `languagenotes` field alone — never `type="proto"` or the two-field-type case. Extracted two more real entries from `sena3.lift` via `scripts/copy-lift-entries.R` into a new `sena3-etymology-proto-and-comment.lift`, chosen to close exactly those gaps in as few entries as possible: one (`1f2f5115-…`) has `type="proto"`, a form, a gloss, and one field type; the other (`011185bb-…`) has `type="borrowed"`, no form, no gloss, and both `comment`+`languagenotes`. `tests/testthat/fixtures/lift2csv_etymology-table/` holds these two plus `lela-teli-empty-lexicon.lift` and `sena3-single-entry-plant.lift` (entries exist, zero etymologies), following the reversal-table directory's curation pattern.

Closed the coverage hole the new table opens ([the skill's "a new table opens a coverage hole"](../.claude/skills/adding-a-lift-field/SKILL.md#if-it-needs-a-table-of-its-own)): cross-referencing every existing `csv2lift/` fixture directory's `entries.csv` guids against the new table's own reader output found two directories whose approved `.lift` snapshots were silently missing real etymology data — `sena3-gloss-initial-b/` (all 5) and `sena3-multiple-senses-per-entry/` (1, the `011185bb` two-field-type entry). Added `etymologies.csv` to both from the reader's own output and reviewed both `.new` diffs by hand before accepting: each added exactly the `<etymology>` elements the source always had, nothing else moved. `test-lift2csv.R`'s two hardcoded expectations (the `sena3-gloss-initial-b` filename list and the unrecognised-`--table`-name error's list of valid names) needed updating for the same reason B1 hit this — both are now `etymologies`-aware.

Registered last in `R/table_registry.R`, after `reversals`, matching `lift.rng`'s own declared order for `entry-content` (`<etymology>` is its last child) — not because anything depends on it: `attach_etymologies_to_lift()` only ever looks entries up by guid, so this is a readability choice, like reversals-after-examples. Real FLEx output actually emits `<etymology>` *before* `<sense>`, so this tool's output deliberately disagrees with the source here — the same kind of divergence already accepted for `<example>` vs `<note>` ([Sense Table](../SPEC.md#sense-table)). Within one `<etymology>`, child order is `<form>`, `<gloss>`, `<field>`, chosen to match the source's own observed order rather than `lift.rng`'s declared one (which lists the field-bearing `extensible-content` first).

Round-trip spot check on the full `sena3.lift` (105 etymologies) confirmed lossless: identical counts of etymologies, `@type` values, field types, glosses, and forms before and after. SPEC.md gained its own [Etymology Table](../SPEC.md#etymology-table) section (modelled on [Pronunciation Table](../SPEC.md#pronunciation-table)), plus: the reserved-prefix list and cross-table collision bullet under [Column Classification](../SPEC.md#column-classification) (`gloss_<lang>` is deliberately shared with the sense table, and `reversal_`/`etymology_` were both missing from the prefix list — the former a pre-existing B1 gap, fixed in passing since this edit already touched the line), the positionally-keyed-row and required-attribute bullets under [Structural Rules](../SPEC.md#structural-rules-csv2lift-direction), the Entry Table canonical-child-order paragraph, and removal of "etymology" from [Not Yet Specified](../SPEC.md#not-yet-specified)'s entry-sub-elements bullet. No README change — `--table-dir` and the registry already cover every table generically.
...
1. `Rscript -e 'devtools::test(filter = "<table>-table")'` — read direction. A brand-new fixture auto-creates its snapshot with a WARN, so **read the file**; that absence is the red.
...
3. `Rscript -e 'testthat::snapshot_accept("<name>-table/")'` (trailing slash required) and the same for `join-sense-entry-table/` where the level reaches that view; re-run to confirm the accept took.
...
5. `Rscript -e 'devtools::test(filter = "csv2lift")'` — write direction; review the `.lift` diff before accepting.
...
- **Adding a table to an *existing* export's round trip is a file drop, not a code change.** Dropping `reversals.csv` into a fixture's export directory is enough for `test-csv2lift.R` to pick it up. Read the resulting snapshot diff rather than accepting it — see [B1](#phase-b--new-tables-whose-data-is-already-in-the-repo).

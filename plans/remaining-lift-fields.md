...
1. **A2's column name.** The store-vs-derive-vs-skip half is **settled** (store verbatim, per the FLEx documentation — see A2 and [redundant-columns-and-entry-id.md](old/redundant-columns-and-entry-id.md)); only the name is open. `entry_lift_id` is the suggestion (descriptive, and `entry_id` is taken by the guid). Renaming later is expensive: [the skill spells out](../.claude/skills/adding-a-lift-field/SKILL.md#documentation) that a column name is a public contract touching reader, classifier, every fixture header, three snapshot directories and SPEC.md.

...
### Phase T — the table folder convention (next)

Out of alphabetical order deliberately: **T1 runs before B1/B2**, and is the only increment here that changes no data model, no CSV column and no snapshot content.
...
C1 needs `attach_order` to be explicit anyway — its traits table must attach **last**, after variants and relations exist ([C1](#phase-c--traits-then-the-two-trait-bearing-tables)) — so the registry is where that constraint stops being a comment about call order and becomes data.
...
--tables fixtures/csv2lift/Sena3_gloss_initial_b   → …_entries.csv, …_senses.csv, …
```
...
4. A folder-shaped fixture for the trailing-slash case — one stem copied into `tests/testthat/fixtures/csv2lift-folder/Sena3_gloss_initial_b/{entries,senses,examples}.csv` — so both halves of the discovery rule are covered, not just the flat one the existing fixtures happen to use.
...
The one open decision is the prefix/separator rule itself — see [decision 5](#decisions-to-settle).
...
5. ~~**T1's prefix rule.**~~ **Settled**: one `--tables <prefix>` where a trailing path separator switches from `<prefix>_<name>.csv` to `<name>.csv` — one helper, one code path, both the flat fixture layout and a user's per-export folder covered. Everything else in T1 is a build-it decision, not an open one.
...
**After [T1](#phase-t--the-table-folder-convention-next), five of those nine collapse into one registry row.** The flag, the attach call and its position, the `requires` guard, the test-loop glob and the SPEC.md CLI bullet all become fields in `R/table_registry.R`; the README's per-flag example becomes unnecessary because `--tables` covers every table at once. What is left per new table is the genuinely per-table work: the three `R/` files, the `scripts/lift2csv_<name>-table.R` one-liner, the test file, the fixture directory, the registry row, and the SPEC.md section describing the table's columns and keys. That is the reason T1 comes before the six remaining tables rather than after them.

...
Built incrementally as design decisions are actually made through development; it does not speculate ahead of what is implemented. See "Not Yet Specified" (§10) for what is deliberately left undesigned.
...
Each table section (§3–§6) states only its own column names and any deviation from these shared rules.
...
- **Typed notes are named `note_<type>_<lang>` at every level**, deliberately sharing one column name across the entry (§3) and sense (§4) tables rather than taking a per-level prefix (as untyped notes do, `note_` vs `general_note_`) or a FLEx-label-derived name. Two reasons:
  ...
  The consequence is that one typed-note column name can occur on both tables; §7 covers how the join view resolves that.
...
- **Empty optional elements are never emitted**: an element is written only when at least one of its columns is non-blank for that row. This covers entry-level lexical-unit/citation/note/typed note/morph_type/field (§3), sense-level definition/note/typed note/field (§4), and a whole `<pronunciation>` whose row has neither a form nor a `media_href` (§5).
  **The rule stops at the row.** In a positionally-keyed table the row *is* the element's identity, so an all-blank row still emits its element: the example table (§6) writes one `<example>` per row even when every column is blank, because dropping it would shift the position — and therefore the identity — of that example's surviving siblings. `<pronunciation>` (§5) still omits a whole blank row rather than emitting `<pronunciation/>`; no fixture has ever exercised that branch, so the two positionally-keyed tables currently disagree on this point. This is a known inconsistency, not a considered design choice — see §10.
...
**csv2lift direction:** implemented by `attach_senses_to_lift(doc, sense_table)` (`R/csv2lift_sense.R`), using `classify_sense_columns()` (`R/sense_helpers.R`) to classify columns per the algorithm above. LIFT uses `id` (not `guid`) for `<sense>`; it is emitted from `sense_guid`, omitted if blank. `order` is emitted from `sense_order` and omitted when blank *or when the column is absent entirely* — a sense CSV produced before `sense_order` existed still converts, mirroring the entry writer's `dateCreated`/`dateModified` guard (§3). Canonical child order (§2.3): `<grammatical-info>`, `<gloss>` (one per non-blank `gloss_<lang>` column, added directly under `<sense>`), `<definition>`, `<note>` (from `general_note_<lang>` columns), one `<note type=X>` per distinct note type, one `<field type=X>` per distinct field type — grouping every lang for that type into one `<note>`/`<field>` element, never one per lang — then `<example>` elements (§6). One `<sense>` is attached per matching row, in CSV row order — proven by the `sena3_multiple_senses_per_entry` fixture.

Examples are appended by a later pass over the example table (§6), exactly as pronunciations and senses are appended to `<entry>` (§3) — what fixes their position among a sense's children is the order `scripts/csv2lift.R` calls `attach_senses_to_lift()` and `attach_examples_to_lift()`, not this list. This means the emitted order can disagree with FLEx's own child order, and for `<example>` it always does when a sense has both: FLEx puts `<example>` *before* `<note>` (412 senses in `Sena3.lift` are `grammatical-info, gloss, example, note`), but this tool always emits it last. `lift.rng` wraps sense children in `<interleave>` (§2.3), so this is permitted — content is preserved, only position changes — but it is the first case in this data model where the emitted order deliberately disagrees with the source rather than mirroring it.
...
One row per `sense/example`. Foreign key: `sense_guid`. **No primary key** — `<example>` carries no `id` or `guid` in LIFT, the same situation as `<pronunciation>` (§5) one level deeper: the only thing identifying an example is its position among all `sense/example` elements in the document. Row order is therefore load-bearing in both directions, per §2.1.
...
This is a denormalized *view*, not a base table — csv2lift does not consume this shape directly. Building a LIFT file from it would require grouping/de-duplicating the entry-level columns back out; the tool deliberately does not do this. Instead, csv2lift takes normalized per-table CSVs (§8).
...
The example table (§6) never reaches this view — it joins entry and sense only — so its `note_reference_<lang>` column cannot clash with anything here even though the name is shared with the typed-note scheme.

## 8. csv2lift CLI shape
...
- **`<note>` cardinality per type is a fixture fact, not a schema guarantee**: `lift.rng` wraps `note` in `zeroOrMore`, so LIFT permits more than one `<note>` of the same `type` on one parent. Every fixture has at most one per type per parent (entry-, sense-, and example-level), which is what permits the flat `note_<type>_<lang>` column (§3, §4, §6); a source with two same-typed notes on one parent is unhandled.
- **A duplicate `<translation>` on one `<example>` loses the second one.** `lift.rng`'s own schematron on `translation-content` asserts "Translations should be of different types", so this is invalid LIFT that FLEx emitted rather than a shape being declined; it occurs once in `Sena3.lift` (entry `d5cb3ce5-…`), and `extract_example_translation()` (§6) warns on stderr and keeps only the first.
- `sense/subsense/note` and `sense/subsense/example` remain unread — both are excluded by the direct-child axes used at the sense/example levels (§4, §6), since `<subsense>` itself is unsupported (see below) and its senses have no row in the sense table.
...
- Within `<example>` (§6): example-level `<field>`/`<trait>`/`<annotation>` children and the `dateCreated`/`dateModified` attributes `extensible-content` allows. None of these appear in any fixture.

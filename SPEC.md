...
| `note_<type>_<lang>`  | `entry/note[@type]/form`                 | typed notes; split on the **last** underscore, same scheme as custom fields (see below) |
...
3. Else matches `^note_.+_[^_]+$` (i.e. at least two underscores after the `note_` prefix) → typed note; split on the **last** underscore: type = everything between `note_` and the last underscore, lang = everything after. Must precede step 4, or a typed note's type would be read as part of the writing system.
4. Else matches `^note_(.+)$` → note (untyped), lang = capture group.
5. Else contains no underscore → lexical-unit, lang = column name.
6. Else → custom field; split on the **last** underscore: field type = everything before, lang = everything after.

Known limitation (accepted, not solved generally): a writing-system code containing an underscore, or a custom field literally named `citation` or `note`, will misclassify — and, since typed notes now share the `note_` prefix's namespace, a custom field whose type itself begins with `note_` (e.g. a field named `note_foo`) will misclassify as a typed note.

`<note>` is read/written untyped ([not(@type)]) into `note_<lang>` and typed ([@type]) into `note_<type>_<lang>`, mirroring custom `<field>` handling. LIFT/FLEx reuses the `<note>` element for other, unrelated fields distinguished by `type` (e.g. `<note type="restrictions">` is FLEx's separate "Restrictions" field, not its generic "Note" field — confirmed against real FieldWorks-exported data, where filtering for the Note field in FLEx does not surface `restrictions`-typed notes). The shared `note_<type>_<lang>` naming — rather than a FLEx-label-derived name — is deliberate; see §4 for the rationale, which applies equally at entry level. `lift.rng` wraps `note` in `zeroOrMore`, but every fixture has at most one `<note>` of a given type per entry, which is what permits the flat one-column-per-type-per-lang shape (see §9).
...
- Canonical child element order (not schema-required — `lift.rng` wraps the entry's children in an `<interleave>` — but always emitted this way for readability): `<lexical-unit>`, `<trait name="morph-type">`, `<citation>`, `<note>`, one `<note type=X>` per distinct note type, one `<field type=X>` per distinct field type — grouping every lang for that type into one `<note>`/`<field>` element, never one per lang — then `<pronunciation>` elements (§5), then `<sense>` elements (§4). Pronunciations and senses are appended by later passes over their own tables, so what fixes their position is the order `scripts/csv2lift.R` calls `attach_pronunciations_to_lift()` and `attach_senses_to_lift()`.
- Within `<lexical-unit>`, `<citation>`, `<note>`, and each typed `<note>`/`<field>`, `<form lang>` children are emitted in the CSV's own column order (§2) — never re-sorted.
- An entry with no data for a given optional element (lexical-unit/citation/note/typed note/morph_type/field) simply omits that element — no empty elements are emitted.
...
| `note_<type>_<lang>`                       | `sense/note[@type]/form`   | typed notes; split on the **last** underscore, same scheme as custom fields — and shared with the entry-level typed-note column name (§3), see rationale below |
...
5. Else matches `^note_.+_[^_]+$` (i.e. at least two underscores after the `note_` prefix) → typed note; split on the **last** underscore: type = everything between `note_` and the last underscore, lang = everything after. Order vs step 4 is free (the two prefixes are disjoint); order vs step 6 is load-bearing, or `note_phonology_en` becomes `<field type="note_phonology">`.
6. Else → custom field; split on the **last** underscore: field type = everything before, lang = everything after.

Known limitation (accepted, not solved generally): a writing-system code containing an underscore, or a custom field literally named `gloss`, `definition`, or `general_note`, will misclassify — the same limitation entry columns have (§3) — and, since typed notes share the `note_` prefix's namespace, a custom field whose type itself begins with `note_` will misclassify as a typed note.

**Naming: `note_<type>_<lang>` is deliberately shared with the entry-level typed-note column (§3)**, rather than following the general/entry-level split (`general_note_` vs `note_`) or a FLEx-label-derived name. Two reasons:
...
Typed notes are the opposite case: `note_<type>_<lang>` (§3, §4) is *deliberately* shared across levels, and does clash on real data — `Sena3.lift` has both an entry-level and a sense-level `restrictions` note, so its join snapshot contains `note_restrictions_en_sense` and `note_restrictions_en_entry`. This is not a bug to "fix" by giving typed notes a per-level prefix (see §4's naming rationale); the `_sense`/`_entry` suffixing above is exactly the mechanism for two identically-named fields at different levels, and this is the first real data to exercise it.
...
- **Sibling re-order on distinct attribute-keyed elements** (§2): the global first-appearance column order can disagree with a given parent's own element order for custom `<field type>`s and typed `<note type>`s, and csv2lift re-orders to match the column order on round-trip. Real figure: 47 senses in `Sena3.lift` hold `<note type="phonology">` before `<note type="sociolinguistics">` locally, but the CSV's global column order (established by whichever type's `(type, lang)` pair first appeared anywhere in the document) puts `note_sociolinguistics_en` first, so all 47 re-emit as `(sociolinguistics, phonology)`. `<field type>` has the identical latent exposure — the CSV column-naming scheme is the same — it has simply never fired on any fixture's field data. Do **not** "fix" this by sorting columns; that would violate the first-appearance rule (§2) and only works here because FLEx happens to sort its own typed-note siblings alphabetically.
...
- Inline `<span lang>` markup (§2) is preserved except for: a span attribute other than `lang` (dropped), a nested span (would not round-trip), and ordinary text that itself contains a literal `<span lang="…">…</span>` string (turned into real markup on the way back). None of these occur in any fixture.

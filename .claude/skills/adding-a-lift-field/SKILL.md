...
Procedure for taking a LIFT element from unsupported to fully round-tripping. Five worked examples run throughout:
...
- **typed `<note type=…>` at entry *and* sense level** (see `plans/typed-notes-entry-and-sense.md`), the first field keyed by an **open attribute** rather than having a fixed identity. That breaks two rules that hold everywhere else: the column is named from the tag and attribute rather than the FLEx label (step 0), and the *same* column name is deliberately shared across both levels rather than kept distinct (step 1). It is also the first field that round-trips content-faithfully but not order-faithfully (step 5).
...
**Sometimes the clash is the correct answer.** Typed notes deliberately share `note_<type>_<lang>` across entry and sense level, and `note_restrictions_en` really does exist on both tables in `Sena3.lift` (61 entry-level, 1 sense-level), so the join view emits `note_restrictions_en_sense` / `note_restrictions_en_entry`. That is right, not a defect. The untyped split (`note_` vs `general_note_`) earned its keep because FLEx genuinely labels those two fields differently — but a *type's* label is identical at both levels: entry "Restrictions" and sense "Restrictions" are both just "Restrictions", and an artificial per-level prefix would mark a distinction FLEx does not make. Ask which case you are in: **different fields that happen to share a tag → different names; the same field at two levels → the same name, and let `suffix =` do the job it exists for.** Record the reasoning in SPEC.md §6, or someone will later "tidy" the names apart and break the classifier.
...
- **Then check the join view** if the new column's level appears in it. `join_sense_entry()` is a *third* consumer of column names that neither classifier sees (step 1), and it is where a cross-level clash surfaces — as `.x`/`.y` columns in an approved CSV. Adding sense-level notes forced the explicit `suffix =` there; the later rename to `general_note_<lang>` stopped the clash happening at all, and the suffix stayed as the fallback for the case still possible — which typed notes then made fire on real data, by design (step 1).
...
- **Re-derive every figure at the moment you write it into SPEC.md.** Counts are load-bearing claims, and a plan's tally may predate the code it describes or simply be wrong — `plans/typed-notes-entry-and-sense.md` asserted 44 reordered senses where the fixture has 47 (45 only if you wrongly require the two notes to be adjacent). One `xml2` loop over the fixture costs seconds; a wrong number in the spec outlives the change that introduced it.

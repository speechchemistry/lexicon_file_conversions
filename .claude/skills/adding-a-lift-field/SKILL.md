...
**Design ([Decide where it lives](#decide-where-it-lives-a-column-or-a-table-of-its-own)). A new table's parent doesn't have to be `<entry>`.** `<pronunciation>` hangs off entries, but the same reasoning applies one level down — `sense/reversal` is `zeroOrMore` under `<sense>`, not `<entry>`, the same shape `sense/example` turned out to have ([SPEC.md's Example Table](../../../SPEC.md#example-table), `plans/sense-level-example-sentences.md`); it's now built too ([SPEC.md's Reversal Table](../../../SPEC.md#reversal-table), `plans/remaining-lift-fields.md`'s B1). Name the new table's foreign key after whichever parent it hangs off — `entry_id` for an entry-parented table, `sense_guid` for a sense-parented one — mirroring how `sense_table()` itself uses `entry_id` as its own FK. The parent can even be a table you're building in the same change, not only an existing one — the example table's `translation_type` turned out not to need this (one Translation per Example, per the FLEx UI), but a genuinely `zeroOrMore` child of a table you're building in the same change would. This choice determines the lookup xpath and the attach-call ordering below.
...
Also route the new table through the shared `format_table_csv()`/`drop_empty_columns()` (`R/table_csv.R`): a column blank in every row is never emitted, so a lexicon with none of the new element's instances snapshots as an empty file automatically, rather than needing its own `nrow(table) == 0` special case — both via `--table <name>` to stdout and, for the write-fixture step below, via `--table-dir`.
